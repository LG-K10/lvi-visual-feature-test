...
class mapOptimization : public ParamServer
{
    public:
        ...
        mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lvi_sam_mapOptimization" , options)
        {
            ...
            pubKeyPoses             = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/trajectory", 1);
            pubLaserCloudSurround   = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_global", 1);
            pubOdomAftMappedROS     = create_publisher<nav_msgs::msg::Odometry>      (PROJECT_NAME + "/lidar/mapping/odometry", 1);
            pubPath                 = create_publisher<nav_msgs::msg::Path>          (PROJECT_NAME + "/lidar/mapping/path", 1);

            subLaserCloudInfo       = create_subscription<lvi_sam::msg::CloudInfo>(PROJECT_NAME + "/lidar/feature/cloud_info", 5,std::bind(&mapOptimization::laserCloudInfoHandler, this,std::placeholders::_1));
            subGPS                  = create_subscription<nav_msgs::msg::Odometry>(gpsTopic,50,std::bind(&mapOptimization::gpsHandler, this,std::placeholders::_1));
            subLoopInfo             = create_subscription<std_msgs::msg::Float64MultiArray>(PROJECT_NAME + "/vins/loop/match_frame", 5,std::bind(&mapOptimization::loopHandler, this,std::placeholders::_1));

            pubHistoryKeyFrames     = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/loop_closure_history_cloud", 1);
            pubIcpKeyFrames         = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/loop_closure_corrected_cloud", 1);
            pubLoopConstraintEdge   = create_publisher<visualization_msgs::msg::MarkerArray>(PROJECT_NAME + "/lidar/mapping/loop_closure_constraints", 1);

            pubRecentKeyFrames      = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_local", 1);
            pubRecentKeyFrame       = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/cloud_registered", 1);
            pubRecentKeyFrame       = create_publisher<sensor_msgs::msg::PointCloud2>(PROJECT_NAME + "/lidar/mapping/cloud_registered_raw", 1);
...
int main(int argc , char **argv)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\033[1;32m----> Lidar Map Optimization Started.\033[0m");
